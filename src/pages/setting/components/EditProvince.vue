<template>
  <div class="text-xs-center">
    <v-card>
      <title-page>Cập nhật tỉnh thành</title-page>
...
export default {
  ...
  methods: {
    ...
    async update(model) {
      try {
        let rs = await this.Update([url.province.route, model]);
        if (typeof rs == "string") {
          ...
          window.location.href = "#/Setting/Province";
        }
      } catch (error) {
    ...
    async remove() {
      try {
        let rs = await this.DeleteById([url.province.id, this.form.id]);
        if (typeof rs == "string") {
          window.getApp.showMessage(rs, messageResult.Error);
        } else {
          window.getApp.showMessage(
    ...
    async getById(id) {
      try {
        let rs = await this.GetById([url.province.id, id]);
        if (typeof rs == "object") {
          this.form.id = rs.id;
