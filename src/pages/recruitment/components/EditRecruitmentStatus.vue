<template>
  <div class="text-xs-center">
    <v-card>
      <title-page>Cập nhật trạng thái tuyển dụng</title-page>
      <v-container>
...
export default {
  ...
  methods: {
    ...
    async update(model) {
      try {
        let rs = await this.Update([url.recruitment_status.route, model]);
        if (typeof rs == "string") {
    ...
    async getById(id) {
      try {
        let rs = await this.GetById([url.recruitment_status.id, id]);
        if (typeof rs == "object") {
          this.form.id = rs.id;
