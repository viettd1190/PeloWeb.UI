<template>
  <div class="text-xs-center">
    <v-card>
      <title-page>Cập nhật trạng thái công việc</title-page>
      <v-container>
...
export default {
  ...
  methods: {
    ...
    async update(model) {
      try {
        let rs = await this.Update([url.task_status.route, model]);
        if (typeof rs == "string") {
    ...
    async remove() {
      try {
        let rs = await this.DeleteById([url.task_status.id, this.form.id]);
        if (typeof rs == "string") {
          window.getApp.showMessage(rs, messageResult.Error);
        } else {
          window.getApp.showMessage(
    ...
    async getById(id) {
      try {
        let rs = await this.GetById([url.task_status.id, id]);
        if (typeof rs == "object") {
          this.form.id = rs.id;
