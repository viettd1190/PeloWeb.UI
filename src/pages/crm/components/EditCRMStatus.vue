...
export default {
  ...
  methods: {
    ...
    async update(model) {
      try {
        let rs = await this.Update([url.crm_status.route, model]);
        if (typeof rs == "string") {
    ...
    async remove() {
      try {
        let rs = await this.DeleteById([url.crm_status.id, this.form.id]);
        if (typeof rs == "string") {
          window.getApp.showMessage(rs, messageResult.Error);
        } else {
          window.getApp.showMessage(
    ...
    async getById(id) {
      try {
        let rs = await this.GetById([url.crm_status.id, id]);
        if (typeof rs == "object") {
          this.form.id = rs.id;
