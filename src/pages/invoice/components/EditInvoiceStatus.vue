...
export default {
  ...
  methods: {
    ...
    async update(model) {
      try {
        let rs = await this.Update([url.invoice_status.route, model]);
        if (typeof rs == "string") {
    ...
    async remove() {
      try {
        let rs = await this.DeleteById([url.invoice_status.id, this.form.id]);
        if (typeof rs == "string") {
          window.getApp.showMessage(rs, messageResult.Error);
        } else {
          window.getApp.showMessage(
    ...
    async getById(id) {
      try {
        let rs = await this.GetById([url.invoice_status.id, id]);
        if (typeof rs == "object") {
          this.form.id = rs.id;
